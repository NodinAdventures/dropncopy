...
const BUILD_ID = "2026-09-21-v26.17.12-factcheck-words-numbers-only";
...
jnjLog("BOOT", "v26.17.12 boot. BUILD_ID:", "2026-09-21-v26.17.12-factcheck-words-numbers-only");
...
try {
  ...
  badge.textContent = `v26.17.12 · ${BUILD_ID}`;
  ...
  async function jnjPollLbState() {
    try {
      ...
      if (lbOn) {
        badge.textContent = `v26.17.12 · A:${a} B:${b}`;
      } else {
        badge.textContent = `v26.17.12 · A:${a} (single key)`;
      }
    } catch {}
  }
  jnjPollLbState();
...
async function jnjHandleFiles(input) {
  ...
  try {
    ...
    // v26.14.1: PUT THE SORT BACK. v26.14 removal was based on the wrong
    // assumption — the browser DOES scramble files on large multi-photo
    // drops even when they're all named 001, 002, 003 in sequence. Confirmed
    // in FILE-47 test log: photo 018 arrived before photo 001. The camera
    // saves files in order, but Chrome hands us the File objects in whatever
    // order the OS filesystem read them, which is NOT numeric on big drops.
    // Sorting by filename restores true shot order.
    photos.sort((a, b) => (a.name || "").localeCompare(b.name || "", undefined, { numeric: true, sensitivity: "base" }));
    jnjLog("PHOTO-ORDER", `Sorted ${photos.length} photos by filename. First:`, photos[0]?.name, "Last:", photos[photos.length-1]?.name);
  ...
  for (let _rIdx = 0; _rIdx < jnjState.items.length; _rIdx++) {
    ...
    if (currentSheetKey !== _lastRenderedSheet) {
      ...
      header.innerHTML = `
        ...
        <button class="jnj-renumber-btn" data-sheet="${currentSheetKey}" data-first="${minN}" title="Fix systematic misreads by resetting all lot numbers on this sheet in sequence">✏️ Fix lot #s on this sheet</button>
      `;
